...
Nota di audit: `dotnet list package --vulnerable --include-transitive` segnala `SQLitePCLRaw.lib.e_sqlite3 2.1.11` come advisory High transitivo di `Microsoft.Data.Sqlite 10.0.0`. La versione stabile disponibile nel contesto non offre una sostituzione risolutiva; il progetto mantiene SQLite funzionale, ma serve una decisione di upgrade/provider prima di considerarlo pronto per uso produttivo.
